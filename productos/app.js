...
app.post('/productos', (req, res) => {
  ...
  db.query('INSERT INTO productos (nombre, descripcion, marca, stock) VALUES (?, ?, ?, ?)',
...
app.put('/productos/:id', (req, res) => {
  const { id } = req.params;
  const { nombre, descripcion, marca, stock } = req.body;
  db.query('UPDATE productos SET nombre = ?, descripcion = ?, marca = ?, stock = ? WHERE id = ?',
    [nombre, descripcion, marca, stock, id],
    (err, result) => {
      if (err) throw err;
